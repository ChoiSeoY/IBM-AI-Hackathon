...
from model import analyze_self_intro, qanda_self_intro, mkq_self_intro
...
# API 엔드포인트 (POST 요청) -면접
@app.post("/qanda")
async def qanda_intro(request: SelfIntroRequest):
    feedback = qanda_self_intro(request.user_text)
    return {"feedback": feedback}

# API 엔드포인트 (POST 요청) -예상질문생성
@app.post("/mkq")
async def mkq_intro(request: SelfIntroRequest):
    feedback = mkq_self_intro(request.user_text)
    return {"feedback": feedback}
